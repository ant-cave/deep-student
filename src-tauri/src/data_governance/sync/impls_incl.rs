impl SyncManager {
    ...
    pub async fn download_changes(
        ...
        // Phase 4: 优先从 UnifiedSyncManifest 读取变更信息，避免 list_outcome
        if let Some(unified) = self.download_unified_manifest(storage).await? {
            if unified.changes_meta.values().any(|m| m.changes_archive.is_some()) {
                return self.download_changes_from_manifest(storage, &unified, since_version, per_db_since).await;
            }
        }
        // 回退：list_outcome 目录遍历（旧格式）
        let list = storage
    ...
    async fn max_cloud_seq_for_device(
        ...
        // Phase 4: 优先从 UnifiedSyncManifest 读取已发布 seq
        if let Some(unified) = self.download_unified_manifest(storage).await? {
            if let Some(meta) = unified.changes_meta.get(device_id) {
                if meta.published_max_seq > 0 {
                    return Ok(meta.published_max_seq);
                }
            }
        }
        // 回退：列出云端变更文件
...
impl SyncManager {
    ...
    pub async fn sync_workspace_databases_with_progress(
        ...
        self.sync_workspace_databases_with_manifest(storage, active_dir, direction, progress, cloud_manifest).await
    }

    /// [P1] 接受预加载 manifest 的变体，避免内部 GET
    pub async fn sync_workspace_databases_with_manifest(
        &self,
        storage: &dyn CloudStorage,
        active_dir: &std::path::Path,
        direction: SyncDirection,
        progress: Option<FileTransferProgressCallback>,
        mut cloud_manifest: WorkspacesManifest,
    ) -> Result<(), SyncError> {
    ...
    pub(crate) async fn download_workspaces_manifest(
        &self,
        storage: &dyn CloudStorage,
    ...
    pub async fn sync_vfs_blobs_with_progress(
        ...
        self.sync_vfs_blobs_with_manifest(storage, blobs_dir, direction, progress, cloud_manifest).await
    }

    pub async fn sync_vfs_blobs_with_manifest(
        &self,
        storage: &dyn CloudStorage,
        blobs_dir: &std::path::Path,
        direction: SyncDirection,
        progress: Option<FileTransferProgressCallback>,
        cloud_manifest: BlobsManifest,
    ) -> Result<BlobSyncOutcome, SyncError> {
        if !blobs_dir.exists() {
    ...
    pub(crate) async fn download_blobs_manifest(
        &self,
        storage: &dyn CloudStorage,
    ...
    pub async fn sync_asset_directories_with_progress(
        ...
        self.sync_asset_directories_with_manifest(storage, active_dir, app_data_dir, direction, progress, cloud_manifest).await
    }

    pub async fn sync_asset_directories_with_manifest(
        &self,
        storage: &dyn CloudStorage,
        active_dir: &std::path::Path,
        app_data_dir: &std::path::Path,
        direction: SyncDirection,
        progress: Option<FileTransferProgressCallback>,
        cloud_manifest: AssetDirsManifest,
    ) -> Result<AssetSyncOutcome, SyncError> {
        ...
        if direction != SyncDirection::Download {
            // 收集需要上传的文件
            let mut files_to_upload: Vec<(String, std::path::PathBuf, u64)> = Vec::new();
            for (key, (path, sha256, size, local_updated_at)) in &local_files {
                ...
                if should_upload {
                    files_to_upload.push((key.clone(), path.clone(), *size));
                }
            }

            // 小批量：逐个上传（archive_seq = 0 表示旧版模式）
            let is_small = files_to_upload.len() < 3
                && files_to_upload.iter().map(|(_, _, s)| *s).sum::<u64>() < 1024 * 1024;

            if is_small || files_to_upload.is_empty() {
                for (key, path, size) in &files_to_upload {
                    let remote_key = format!("{}/{}", Self::ASSETS_CLOUD_PREFIX, key);
                    let tp = Self::file_transfer_progress(progress.as_ref(), format!("资产文件 {}", key));
                    match storage.put_file(&remote_key, path, tp).await {
                        Ok(_) => {
                            new_manifest.entries.insert(
                                key.clone(),
                                AssetFileEntry {
                                    sha256: crate::backup_common::calculate_file_hash(path).unwrap_or_default(),
                                    size: *size,
                                    updated_at: Self::file_mtime_rfc3339(path),
                                    archive_seq: 0,
                                },
                ...
                // 大批量：归档上传（Phase 2）
                let batches = Self::split_into_archive_batches(&files_to_upload);
                let next_seq = cloud_manifest.archives.iter().map(|a| a.seq).max().unwrap_or(0) + 1;
                for (batch_idx, batch) in batches.into_iter().enumerate() {
                    let archive_seq = next_seq + batch_idx as u32;
                    ...
                    let zip_data = match Self::pack_blobs_zip(&batch) {
                        Ok(d) => d,
                        Err(e) => { for (h, _) in &batch { upload_failures.push(h.clone()); } continue; }
                    };
                    // zstd compress
                    let compressed = match zstd::stream::encode_all(std::io::Cursor::new(&zip_data), 0) {
                        Ok(c) => c,
                    ...
                    let archive_key = format!("{}/assets_archive_{:04}.zip.zst", Self::ASSETS_CLOUD_PREFIX, archive_seq);
                    match storage.put(&archive_key, &payload).await {
                        Ok(_) => {
                            use sha2::{Digest, Sha256};
                            let mut hasher = Sha256::new();
                            ...
                            let archive_hash = format!("{:x}", hasher.finalize());
                            new_manifest.archives.push(ArchiveEntry {
                                seq: archive_seq, sha256: archive_hash,
                                size: payload.len() as u64, blob_count: batch.len(),
                                created_at: chrono::Utc::now().to_rfc3339(),
                            });
                            for (key, path) in &batch {
                                let sha = crate::backup_common::calculate_file_hash(path).unwrap_or_default();
                                let size = std::fs::metadata(path).map(|m| m.len()).unwrap_or(0);
                                new_manifest.entries.insert(key.clone(), AssetFileEntry {
                                    sha256: sha, size, archive_seq,
                                    updated_at: Self::file_mtime_rfc3339(path),
                                });
                            }
                            uploaded += batch.len();
                        }
                        Err(e) => { for (h, _) in &batch { upload_failures.push(h.clone()); } }
                    }
                }
            }
        }
        ...
        if direction != SyncDirection::Upload {
            // 按 archive_seq 分组缺失文件
            let has_archives = !cloud_manifest.archives.is_empty();
            if has_archives {
                let mut missing_by_archive: std::collections::HashMap<u32, Vec<(String, AssetFileEntry)>> = std::collections::HashMap::new();
                let mut missing_individual: Vec<(String, AssetFileEntry)> = Vec::new();
                    ...
                    let should_download = match local_files.get(key) {
                        ...
                        Some((_path, sha256, size, local_updated_at)) => {
                            (*sha256 != entry.sha256 || *size != entry.size)
                                && !Self::local_file_wins(local_updated_at, &entry.updated_at, sha256, &entry.sha256)
                        }
                    };
                    if !should_download { continue; }
                    if entry.archive_seq > 0 {
                        missing_by_archive.entry(entry.archive_seq).or_default().push((key.clone(), entry.clone()));
                    } else {
                        missing_individual.push((key.clone(), entry.clone()));
                    }
                }

                // 下载归档并解压
                for (archive_seq, items) in &missing_by_archive {
                    let archive_key = format!("{}/assets_archive_{:04}.zip.zst", Self::ASSETS_CLOUD_PREFIX, archive_seq);
                    let data = match storage.get(&archive_key).await {
                        Ok(Some(d)) => d,
                        _ => { for (k, _) in items { download_failures.push(k.clone()); } continue; }
                    };
                    let decrypted = match self.decode_payload(&data) {
                        Ok(d) => d,
                        Err(e) => { for (k, _) in items { download_failures.push(k.clone()); } continue; }
                    };
                    // 手动解压，用 asset_local_path_from_key 确保正确路径
                    let zip_bytes = zstd::stream::decode_all(std::io::Cursor::new(&decrypted)).unwrap_or(decrypted);
                    let cursor = std::io::Cursor::new(&zip_bytes);
                    if let Ok(mut archive) = zip::ZipArchive::new(cursor) {
                        let mut extracted = 0usize;
                        for i in 0..archive.len() {
                            if let Ok(mut file) = archive.by_index(i) {
                                let entry_key = file.name().to_string();
                                if let Some(dest) = Self::asset_local_path_from_key(active_dir, app_data_dir, &entry_key) {
                                    if !dest.exists() {
                                        if let Some(parent) = dest.parent() {
                                            let _ = std::fs::create_dir_all(parent);
                                        }
                        ...
                        for (k, _) in items { download_failures.push(k.clone()); }
                    }
                }

                // 逐个下载旧版文件（archive_seq == 0）
                for (key, entry) in &missing_individual {
                    ...
                    let remote_key = format!("{}/{}", Self::ASSETS_CLOUD_PREFIX, key);
                    let tp = Self::file_transfer_progress(progress.as_ref(), format!("资产文件 {}", key));
                    match storage.get_file(&remote_key, &dest, Some(&entry.sha256), tp).await {
                        Ok(_) => downloaded += 1,
                        Err(e) => { let _ = std::fs::remove_file(&dest); download_failures.push(key.clone()); }
                    }
                }
            } else {
                // 旧版：逐个下载
    ...
    pub async fn sync_vfs_blobs_with_tombstones_and_progress(
        ...
        let cloud_manifest = self.download_blobs_manifest(storage).await?;
        self.sync_vfs_blobs_with_tombstones_and_progress_with_manifest(storage, blobs_dir, direction, progress, cloud_manifest).await
    }

    pub async fn sync_vfs_blobs_with_tombstones_and_progress_with_manifest(
        &self,
        storage: &dyn CloudStorage,
        blobs_dir: &std::path::Path,
        direction: SyncDirection,
        ...
        self.sync_vfs_blobs_with_manifest(storage, blobs_dir, direction, progress, cloud_manifest)
            .await
    }
    ...
    pub async fn sync_asset_directories_with_tombstones_and_progress(
        ...
        let cloud_manifest = self.download_assets_manifest(storage).await?;
        self.sync_asset_directories_with_tombstones_and_progress_with_manifest(
            storage, active_dir, app_data_dir, direction, progress, cloud_manifest
        ).await
    }

    pub async fn sync_asset_directories_with_tombstones_and_progress_with_manifest(
        &self,
        storage: &dyn CloudStorage,
        active_dir: &std::path::Path,
        app_data_dir: &std::path::Path,
        ...
        // 2. 走标准同步流程
        self.sync_asset_directories_with_manifest(
            storage,
            active_dir,
            app_data_dir,
            direction,
